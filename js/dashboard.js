...
function initCCL() {
    const form = document.getElementById('cclAnalysisForm');
    if (form) {
        form.addEventListener('submit', handleCCLSubmit);
    }
...
async function handleCCLSubmit(event) {
    event.preventDefault();

    const form = event.target;
    const analyzeBtn = document.getElementById('analyzeBtn');
    const texto = document.getElementById('cclTexto').value.trim();
    const paisOrigen = document.getElementById('cclPaisOrigen').value;
    const nivelDeclarado = document.getElementById('cclNivelDeclarado').value;
    const contexto = document.getElementById('cclContexto').value.trim();

    // Validación básica
    if (texto.split(' ').length < 20) {
        showAlert('Por favor, escribe al menos 20 palabras para obtener un análisis útil', 'warning');
        return;
    }
    ...
    try {
        ...
        if (nivelDeclarado) {
            data.nivel_declarado = nivelDeclarado;
        }

        // Enviar al backend
        const response = await fetch(`${API_URL}/ccl/analizar`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${appState.token}`
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.message || 'Error al analizar el texto');
        }

        const result = await response.json();

        // Mostrar resultados
        displayCCLResults(result.data);
        ...
        // Scroll a resultados
        document.getElementById('cclResults').scrollIntoView({ behavior: 'smooth' });

        showAlert('¡Análisis completado exitosamente!', 'success');

    } catch (error) {
        console.error('Error al analizar texto:', error);
        showAlert(error.message || 'Error al analizar el texto. Por favor, intenta de nuevo.', 'error');
    } finally {
        // Restaurar botón
        analyzeBtn.disabled = false;
        analyzeBtn.innerHTML = '<i class="fas fa-search"></i> Analizar Texto';
    }
}

function displayCCLResults(data) {
    const resultsContainer = document.getElementById('cclResults');
    resultsContainer.style.display = 'block';

    // Diagnóstico Lingüístico
    const diagnostico = data.diagnostico_linguistico_emocional || {};
    document.getElementById('resultNivel').textContent = diagnostico.nivel_probable || '-';
    document.getElementById('resultEmocion').textContent = diagnostico.estado_emocional_dominante || '-';
    document.getElementById('resultRecursos').textContent =
        (diagnostico.recursos_discursivos || []).join(', ') || '-';

    // Hipótesis clínicas
    const hipotesisList = document.getElementById('resultHipotesis');
    hipotesisList.innerHTML = '';
    (diagnostico.hipotesis_clinica_linguistica || []).forEach(hip => {
        const li = document.createElement('li');
        li.textContent = hip;
        hipotesisList.appendChild(li);
    });

    // Radiografía Cultural
    const radiografia = data.radiografia_cultural || {};
    document.getElementById('resultReferentesOrigen').textContent =
        (radiografia.referentes_origen || []).join(', ') || 'Ninguno detectado';
    document.getElementById('resultReferentesAcogida').textContent =
        (radiografia.referentes_acogida || []).join(', ') || 'Ninguno detectado';
    document.getElementById('resultTension').textContent = radiografia.tension_dominante || '-';

    // Detección de Bloqueos
    const bloqueos = data.deteccion_bloqueos || {};
    document.getElementById('resultRiesgoBloqueo').textContent = bloqueos.nivel_riesgo_bloqueo || '-';

    const bloqueosDetails = document.getElementById('resultBloqueosDetails');
    if (bloqueos.posibles_bloqueos && bloqueos.posibles_bloqueos.length > 0) {
        bloqueosDetails.innerHTML = '<h4>Bloqueos detectados:</h4><ul>' +
            bloqueos.posibles_bloqueos.map(b => `<li>${b}</li>`).join('') + '</ul>';
    } else {
        bloqueosDetails.innerHTML = '<p style="color: var(--success-color);">No se detectaron bloqueos significativos.</p>';
    }

    // Tareas Recomendadas
    const tareas = data.prescripcion_tareas || {};
    const tareasContainer = document.getElementById('resultTareas');
    tareasContainer.innerHTML = '';

    if (tareas.tareas_recomendadas && tareas.tareas_recomendadas.length > 0) {
        tareas.tareas_recomendadas.forEach(tarea => {
            const tareaCard = document.createElement('div');
            tareaCard.className = 'tarea-card';
            tareaCard.innerHTML = `
                <h4>${tarea.tipo}</h4>
                <p>${tarea.descripcion}</p>
                <small><strong>Objetivo lingüístico:</strong> ${tarea.objetivo_linguistico}</small>
                <small><strong>Objetivo clínico:</strong> ${tarea.objetivo_clinico_cultural}</small>
            `;
            tareasContainer.appendChild(tareaCard);
        });
    } else {
        tareasContainer.innerHTML = '<p>Continúa con tus estudios regulares. Tu nivel es adecuado.</p>';
    }
}

function addToHistory(analysisData) {
    // Guardar en localStorage
    const history = JSON.parse(localStorage.getItem('cclHistory') || '[]');

    const historyItem = {
        id: Date.now(),
        fecha: new Date().toISOString(),
        texto_preview: analysisData.texto ? analysisData.texto.substring(0, 100) + '...' : '',
        nivel: analysisData.diagnostico_linguistico_emocional?.nivel_probable || '-',
        emocion: analysisData.diagnostico_linguistico_emocional?.estado_emocional_dominante || '-',
        resultados_completos: analysisData
    };
    ...
    localStorage.setItem('cclHistory', JSON.stringify(history));

    // Actualizar UI
    loadCCLHistory();
}

function loadCCLHistory() {
    const historyContainer = document.getElementById('cclHistory');
    ...
    if (history.length === 0) {
        historyContainer.innerHTML = `
            <div class="empty-state">
                ...
                <p>Aún no has enviado textos para análisis</p>
            </div>
        `;
        return;
    }
        ...
        historyItem.innerHTML = `
            <div class="history-item-header">
                <span class="history-item-date">
                    <i class="fas fa-calendar"></i> ${formatDate(item.fecha)}
                </span>
            </div>
            <div class="history-item-preview">${item.texto_preview}</div>
            <div class="history-item-results">
                <span class="history-result-tag"><i class="fas fa-graduation-cap"></i> ${item.nivel}</span>
                <span class="history-result-tag"><i class="fas fa-heart"></i> ${item.emocion}</span>
            </div>
        `;

        historyItem.addEventListener('click', () => {
            displayCCLResults(item.resultados_completos);
            document.getElementById('cclResults').scrollIntoView({ behavior: 'smooth' });
        });
